...
if missing_files:
    ...
    # ============================================================
        ...
        with map_c1:
            ...
            map_m = final_orders.groupby("WarehouseLocation").agg(Cost=("ShippingCost", "sum"), Shipments=("OrderID", "count")).reset_index()
            map_merged = pd.merge(hub_coords, map_m, left_on="Warehouse", right_on="WarehouseLocation", how="inner")
            fig_final_map = px.scatter_mapbox(
                map_merged, lat="lat", lon="lon", size="Shipments", color="Region", hover_name="Warehouse",
                hover_data={"Cost": ":$.2f", "Shipments": True, "lat": False, "lon": False},
                zoom=3.6, center=dict(lat=20.5937, lon=78.9629), mapbox_style="open-street-map",
                title="Hub Volume Distribution"
            )
